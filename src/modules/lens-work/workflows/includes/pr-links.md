...
|----------|-------------|---------------|--------------|
| Finish workflow | `lens/{id}/{lane}/p{N}/w/{name}` | `lens/{id}/{lane}/p{N}` | `finish-workflow` |
| Finish phase | `lens/{id}/{lane}/p{N}` | `lens/{id}/{lane}` | `finish-phase` |
| Lead review | `lens/{id}/small` | `lens/{id}/lead` | `open-lead-review` |
| Final PBR | `lens/{id}/lead` | `lens/{id}/base` | `open-final-pbr` |
...
### Lead Review PR (small → lead)

```markdown
## Lead Review: {initiative_name}
...
- **lane-topology.md** — Branch naming patterns used in PR source/target
