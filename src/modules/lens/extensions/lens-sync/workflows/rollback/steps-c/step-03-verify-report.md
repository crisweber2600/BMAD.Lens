...
outputFile: '{docs_output_folder}/lens-sync/rollback-report.md'
---
...
def verify_doc_consistency(docs_path, lens_data):
    ...
    for ms in lens_data.get("microservices", []):
        expected_path = f"{docs_path}/lens-sync/{ms['name']}/"
        consistency["expected_docs"].append(expected_path)
    
    # Find actual docs
    if exists(f"{docs_path}/lens-sync/"):
        for folder in list_dirs(f"{docs_path}/lens-sync/"):
            folder_path = f"{docs_path}/lens-sync/{folder}/"
...
git checkout {backup.tag} -- {lens_config_path}/ {docs_output_folder}/lens-sync/
...
def write_report(report_content, output_path):
    report_path = f"{output_path}/lens-sync/rollback-report.md"
