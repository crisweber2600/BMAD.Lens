...
required_inputs:
  ...
  - path: "_memory/scout-sidecar/analysis/{target}.yaml"
    type: cached_analysis
    
  # Option 2: Analysis summary report
  - path: "{docs_output_folder}/lens-sync/{target}/analysis-summary.md"
    type: summary_report
...
def validate_analysis_input():
    # Check cache first
    cache_path = f"_memory/scout-sidecar/analysis/{target}.yaml"
    if exists(cache_path):
        return load_yaml(cache_path)
    
    # Check summary report
    report_path = f"{docs_output}/lens-sync/{target}/analysis-summary.md"
...
**Create structure:**
```bash
mkdir -p "{docs_path}/lens-sync/{target}/"
```
...
```yaml
existing_docs:
  - path: "{docs_path}/lens-sync/{target}/architecture.md"
    exists: true
    modified: ISO8601
    has_manual_edits: boolean  # check for human markers
    
  - path: "{docs_path}/lens-sync/{target}/api-surface.md"
    exists: true
    modified: ISO8601
    ...
```
