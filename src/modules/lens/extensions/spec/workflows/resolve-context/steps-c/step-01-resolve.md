...
**Read from session:**
```yaml
current_layer: {from Navigator}
layer_path: {from Navigator}
project_root: {from config}
constitution_root: {from SPEC config, default: lens/constitutions}
```
...
This context is merged into `lens_context` for use by all BMAD workflows.
