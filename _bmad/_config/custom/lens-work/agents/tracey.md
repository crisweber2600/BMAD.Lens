...
Phase Status (per lifecycle.yaml phase_order)
├── preplan: {status} (Mary/Analyst, small)
├── businessplan: {status} (John/PM + Sally/UX, small)
├── techplan: {status} (Winston/Architect, small)
├── devproposal: {status} (John/PM, medium)
└── sprintplan: {status} (Bob/SM, large)

Audience Promotions
├── small → medium: {status} (gate: adversarial-review)
├── medium → large: {status} (gate: stakeholder-approval)
└── large → base: {status} (gate: constitution-gate)
...
- audience-promotion (small-to-medium, medium-to-large, large-to-base)
- override (includes reason field)
- fix-state, archive
- compliance-evaluated, constitution-resolved
