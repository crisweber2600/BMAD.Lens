...
## Goal
Ensure this new-service workflow integrates with git-lens state management, workflow tracking, and phase transitions.
...
✓ **Permissions:** User can create service structures  
...
| **finish-workflow** | Final step | Mark complete, link to service |
...
If transitioning: Planning → Implementation
  ...
  ✓ Service created will be marked for Implementation phase
  ✓ After-workflow hook will advance phase if all workflows complete

If transitioning: Analysis → Planning
  ✓ This workflow defines service structure
  ✓ Service created will be marked for Planning phase
```
...
```
Your branch: feature/new-service-{domain}-{service}
  → Tracked by git-lens
...
Your branch: phase/{phase}/initiative/{id}
  ...
  → All services created here are phase-scoped
```

### What happens at completion:

✓ Service creation committed to current branch  
✓ Commit linked to workflow instance  
✓ git-lens records service metadata  
...
Service State Files:
  .lens/domain-map.yaml
    → New service entry added
    → Links to domain
    
  .lens/services/{service-name}/metadata.yaml
    → Service definition
    → Created timestamp
    → Phase context
  ...
  .git-lens/initiatives/{init-id}/services.yaml
    → Service linked to initiative
```
...
→ Proceed to **Step 4: Collect Service Metadata** (original step-01-collect-metadata.md)
