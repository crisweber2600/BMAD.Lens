...
## Goal
Establish the current BMAD phase (Analysis, Planning, Implementation, QA) via the workflow-guide, then map it to this service creation workflow.
...
**Phase** determines:
- What type of service is being created (conceptual vs. implementation)
...
workflow-guide will:
  1. Run step-01-detect-context
     ...
     → Detect active lens (should be Service)
     → Resolve current BMAD phase
     
  2. Run step-02-map-recommendations
     → Map service-lens + detected-phase to workflows
...
## Mapping Phase to Service Creation

### Phase-Specific Behavior

| Phase | Service Type | Actions | Next |
|-------|--------------|---------|------|
| **Analysis** | Conceptual research | Gather requirements, document discovery | Proceed to Planning |
| **Planning** | Planned service | Create product brief, architecture | Proceed to Implementation |
| **Implementation** | Active service | Scaffold code structure, init repo | Proceed to QA/Testing |
| **QA** | Service refinement | Validate contracts, test integrations | Back to Implementation or Finish |

---

## Workflow Recommendation

**For new-service workflow:**

- **If Analysis phase:**  
  → This is the right time to create service definitions  
  → Focus on discovery and requirements capture  
  → Proceed with metadata collection (Step 3)

- **If Planning phase:**  
  → This is the right time to scaffold service  
  → Focus on architecture and structure  
  → Proceed with metadata collection (Step 3)

- **If Implementation phase:**  
  → Consider if a new service is needed mid-implementation  
  → May require git-lens workflow adjustments  
  → Proceed with metadata collection (Step 3) or consult Casey (git-lens agent)

- **If QA phase:**  
  → Creating a new service during QA is unusual  
